...
def main() -> None:
    ...
    dummy_input = torch.randn(1, config.in_channel, 25, 6000).to(device)
    dummy_mask = torch.randn(1, 1, 6000).to(device)

    # flops, params = profile(model, (dummy_input, dummy_mask))
    # print('flops: %.5fG, params: %.5fM' % (flops / 1000000000.0, params / 1000000.0))
